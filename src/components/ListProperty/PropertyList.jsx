...
const PropertyList = () => {
    ...
    const filteredProperties = properties.filter((p) => {
        ...
        const purposeMatch = selectedPurpose === 'all' || p.purpose?.trim().toLowerCase() === selectedPurpose;
                ...
                {filteredProperties.map((item) => (
                    ...
                    >
                        <div className="image-wrapper">
                            <img src={item.imgURL || '/fallback.jpg'} alt={item.addressLine1} />
                            <div className="badges">
                                <span className={`badge ${item.purpose === 'buy' ? 'for-sale' : 'for-rent'}`}>
                            ...
                            <div className="property-detail-info-overlay">
                                ...
                                <p><FaMapMarkerAlt /> {item.addressLine1}, {item.city}</p>
                                <strong>${item.price}</strong>
                                <div className="property-info">
                                    <span><FaBed /> {item.numBedroom}</span>
                                    <span><FaBath /> {item.numBathroom}</span>
                                    <span><FaExpand /> {item.area}</span>
                                </div>
                            </div>
                        </div>
                    </Link>
